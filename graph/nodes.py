...
from tools.order_manager import catat_pesanan_baru, ubah_jadwal_pesanan, batalkan_pesanan, cek_status_pesanan
from data.vector_manager import get_sop_tool
from data.database import ambil_data_pelanggan_lama

# ================================================================
# LAZY INITIALIZATION — Tools dan ToolNode dibuat saat pertama kali
# dipakai, BUKAN saat import. Mencegah load model HuggingFace dan
# build Chroma DB memblokir startup server.
# ================================================================
_daftar_tools = None

def _get_tools():
    global _daftar_tools
    if _daftar_tools is None:
    ...
    return _daftar_tools

# Pekerja 1: Eksekutor Alat
def node_eksekutor_alat(state):
    return ToolNode(_get_tools()).invoke(state)
def node_pemikir(state: AgentState, config: RunnableConfig):
    llm = get_llm()
    llm_dengan_tools = llm.bind_tools(_get_tools())
...
6. ATURAN WAJIB TANYA TIPE AC UNTUK JASA: Jika pelanggan menanyakan jasa (cuci, service, pasang, bongkar, dll) tapi BELUM menyebutkan TIPE AC-nya (Split/Standing/Cassette), KAMU DILARANG LANGSUNG MEMANGGIL TOOL! Kamu WAJIB bertanya dulu: "Boleh diinfokan kak, AC-nya tipe Split, Standing, atau Cassette yaa?" BARU setelah user menjawab tipe-nya, panggil tool dengan kata kunci lengkap (misal: "Cuci AC Split 1 PK").
...
2. WAJIB gunakan tool `cek_status_pesanan` untuk memverifikasi bahwa pesanan tersebut milik pelanggan ini dan melihat statusnya.
3. SYARAT RESCHEDULE: Reschedule bisa dilakukan selama status pesanan masih "pending". Jika status sudah "in_progress" atau "completed" atau "cancelled", tolak dengan sopan: "Mohon maaf kak, pesanan ini sudah tidak bisa diubah jadwalnya karena teknisi sudah dalam perjalanan/sudah selesai."
4. JIKA AMAN: Tanyakan jadwal pengganti yang baru. 
...
1. Tanyakan Nomor Order (contoh: ORD-xxxx) jika user belum menyebutkannya.
2. WAJIB gunakan tool `cek_status_pesanan` untuk memverifikasi bahwa pesanan tersebut milik pelanggan ini dan melihat statusnya.
3. SYARAT PEMBATALAN: Pembatalan bisa dilakukan selama status pesanan masih "pending". Jika status sudah "in_progress" atau "completed" atau "cancelled", tolak dengan sopan: "Mohon maaf kak, pesanan ini sudah tidak bisa dibatalkan karena teknisi sudah dalam perjalanan/sudah selesai/sudah dibatalkan sebelumnya."
4. JIKA AMAN: Tanyakan ALASAN pembatalannya dengan ramah. (Contoh: "Boleh diinfokan kak, alasan pembatalannya agar bisa kami catat?").
